...
- **passive_voice_conversion**: State explicitly in `instruction` whether the doer must be kept or omitted. The `answer` must honour this exactly — "do not omit the doer" means the "by [doer]" phrase is required; "omit the doer" means it must be absent. Only convert sentences that have a clear direct object; if the sentence cannot form a natural passive, state this in the explanation.
